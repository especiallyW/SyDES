...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(
        ...
        return self.attn(
            q_x, k_x, v_x, need_weights=False, attn_mask=attn_mask, key_padding_mask=key_padding_mask
        )[0]
    ...
    def forward(
        ...
        x = q_x + self.ls_1(self.attention(q_x=self.ln_1(q_x), k_x=k_x, v_x=v_x, attn_mask=attn_mask, key_padding_mask=key_padding_mask))
...
class MultimodalTransformer(Transformer):
    ...
    def forward(self, image_embs, text_embs, attn_mask=None, image_key_padding_mask=None):
        ...
        for resblock, cross_attn in zip(self.resblocks, self.cross_attn):
            if self.grad_checkpointing and not torch.jit.is_scripting():
                ...
                text_embs = resblock(text_embs, attn_mask=self.attn_mask[:seq_len, :seq_len])#attn_mask)
                text_embs = cross_attn(text_embs, k_x=image_embs, v_x=image_embs, key_padding_mask=image_key_padding_mask)
